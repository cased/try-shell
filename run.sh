...
if grep -q try-shell docker-compose.yml 2>/dev/null; then
  cp docker-compose.yml $tmpdir/docker-compose.yml
else
  curl -s https://raw.githubusercontent.com/cased/try-shell/main/docker-compose.yml > $tmpdir/docker-compose.yml
fi
